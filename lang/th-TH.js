export default {
  title: {
    home: 'หน้าหลัก',
    commands: 'คำสั่ง',
    support: 'ช่วยเหลือ',
    dark: 'โหมดมืด',
    light: 'โหมดสว่าง'
  },
  footer: {
    desc: 'MrTheBank Discord Bot พัฒนาโดย MrTheBank#5265, เขียนด้วย discord.js',
    links: 'ลิงค์',
    link1: {
      index: 'เว็บไซค์',
      invite: 'เชิญบอท',
      support: 'ช่วยเหลือ',
    },
  },
  index: {
    subtitle: 'บอทอเนกประสงค์สำหรับ<br/>เซิฟเวอร์ดิสคอร์ทของคุณ',
    invite: 'เชิญบอท',
    commands: 'ดูคำสั่ง',
    about_title: 'บอทนี้คืออะไร',
    about_desc: 'บอทนี้คือบอทอเนกประสงค์ที่มีคำสั่งมากมาย ช่วยทำให้ชีวิตง่ายขึ้น',
    about: {
      cmds_easier: 'คำสั่งที่ทำให้ชีวิตง่ายขึ้น',
      cmds_easier_desc: 'บอทของเรามีคำสั่งที่มีประโยชน์มากมาย ไม่ว่าจะเป็น avatar ที่ดึงรูปภาพของคนนั้นมา เป็นต้น',
      meme_generate: 'คำสั่งสร้างมีม',
      meme_generate_desc: 'ท่านสามารถสร้างมีมของท่านง่ายๆด้วยคำสั่งบอทเพียงไม่กี่ขั้นตอน',
    }
  },
  commands: {
    all: 'ทั้งหมด',
    search: 'ค้นหา',
    required_permissions: 'สิทธิ์ที่จำเป็น',
    not_found: 'ไม่พบคำสั่งที่ท่านค้นหา',
    none: 'ไม่มี',
  }
}
